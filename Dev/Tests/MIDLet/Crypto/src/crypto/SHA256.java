...
final class SHA256 {
    ...
    static byte[] hash(byte[] data) throws SHA256Exception {
        if (data.length == 0) {
            throw new SHA256Exception("E-SC-LN"); // Empty data.
        }
    ...
    private static byte[] pad(byte[] data) {
        ...
        byte paddedData[] = new byte[data.length + length + 8];
    ...
    private static int epsilonCapital0(int x) {
        return rotr(x, 2) ^ rotr(x, 13) ^ rotr(x, 22);
    }

    private static int epsilonCapital1(int x) {
        return rotr(x, 6) ^ rotr(x, 11) ^ rotr(x, 25);
    }

    private static int epsilon0(int x) {
        return rotr(x, 7) ^ rotr(x, 18) ^ shr(x, 3);
    }

    private static int epsilon1(int x) {
        return rotr(x, 17) ^ rotr(x, 19) ^ shr(x, 10);
    }

    private static int getInt(byte[] paddedData, int n, int t) {
        int x = n * 64 + t * 4;
